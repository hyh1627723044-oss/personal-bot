...
- 两种模式：文字模式按请求调用 LLM，不建立 WebRTC 或常驻 Pipecat 会话；语音模式点击开始通话后，同时支持麦克风和文字输入。
- 语音模式的打字通过同一条 WebRTC 数据通道发送，和语音识别共用同一份上下文；发送文字会打断当前回复，并生成文字和语音回答。静音只关闭麦克风，仍可打字。
- 切回文字模式会挂断通话并释放麦克风；切到语音模式会停止正在生成的纯文字回复。两种模式的历史分别保留在当前页面，不跨模式自动迁移；重新开始通话会创建新会话，刷新清空所有页面历史。
...
2026-09-09：后端 `python -m pytest -q` 18 项通过，前端 `npm test` 6 项通过，`npm run build` 通过。

后端测试覆盖配置检查、真实本地 WebRTC 建连和释放，以及文字聊天的多轮上下文、请求校验、上游错误、并发限制和取消清理。前端测试覆盖字幕转换、中文 SSE 拆包、错误响应和流提前结束。

浏览器使用真实前后端和本地模拟 OpenAI 兼容服务，验证了发送、流式回复、多轮上下文、停止、重新生成、新对话清空上下文、鉴权错误提示，以及未配置语音服务时的配置提示。模拟服务仅在测试中使用，不代表真实模型质量或供应商可用性。

尚未配置真实 LLM、ASR/TTS 密钥，因此未验证真实模型回复或完整语音通话。构建存在单个 JS 包超过 500 kB 的体积提示；测试另有第三方库弃用提示，均未导致失败。
